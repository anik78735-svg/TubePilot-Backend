 ...
 *
 * IMPORTANT — field-name assumption: this assumes each Video doc has
 * `userId` (ref to User) plus `cloudinaryPublicId` and `cloudinaryAccount`
 * ('cloudinary_1' | 'cloudinary_2'). If your actual Video schema uses
 * different field names, update the `Video.find(...)` query and the
 * `.map()` below to match. Get this wrong and it will silently delete 0
 * Cloudinary files while still deleting the Video docs — you lose the
 * pointer you'd need to clean up the orphaned files later.
 *
